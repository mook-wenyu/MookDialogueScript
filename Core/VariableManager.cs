...
namespace MookDialogueScript
{
    ...
    public class VariableManager
    {
                ...
                foreach (var type in types)
                {
                    try
                    {
                        ...
                        foreach (var property in properties)
                        {
                            try
                            {
                                ...
                                if (attribute != null)
                                {
                                    ...
                                    Action<object> setter = attribute.IsReadOnly ?
                                        (obj) => Console.WriteLine($"变量 '{varName}' 是只读的") :
                            ...
                            {
                                Console.WriteLine($"扫描属性 {property.Name} 时出错: {ex.Message}");
                            }
                        }
                        ...
                        foreach (var field in fields)
                        {
                            try
                            {
                                ...
                                if (attribute != null)
                                {
                                    ...
                                    {
                                        setter = (obj) => Console.WriteLine($"变量 '{varName}' 是只读的");
                                    }
                                    else
                                    {
                            ...
                            {
                                Console.WriteLine($"扫描字段 {field.Name} 时出错: {ex.Message}");
                            }
                        }
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine($"扫描类型 {type.FullName} 时出错: {ex.Message}");
                    }
                }
            }
            catch (Exception ex)
        ...
        public void RegisterObjectProperties(string objectName, object instance)
        {
            ...
            foreach (var property in properties)
            {
                // 变量名为：objectName$propertyName
                string varName = $"{objectName}${property.Name}";
                ...
                {
                    setter = (obj) => Console.WriteLine($"属性 '{varName}' 是只读的");
                }
                else
                {
        ...
        private object ConvertToNativeType(RuntimeValue value)
        {
            switch (value.Type)
            {
                ...
                default:
                    Console.WriteLine($"不支持的运行时值类型: {value.Type}");
                    return null;
            }
        }
        ...
        private RuntimeValue ConvertToRuntimeValue(object value)
        {
            ...
            {
                Console.WriteLine($"不支持的内置变量类型: {value.GetType().Name}");
                return null;
            }
        }
    }
